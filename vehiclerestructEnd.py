...
def get_common_values_and_unique_values(list1, list2):
    print(type(list1), type(list2))
    common_values = [item for item in list1 if item in list2]  # Find the common dictionaries between the two lists
    unique_values_list2 = [item for item in list2 if item not in list1]  # Dictionaries that only appear in list2
    return len(common_values), unique_values_list2
...
for result in results:
    ...
    if 'vehicles.values' in endorsement_keys:
        while inner_index < len(endorsements_json['vehicles.values']['values']):
            ...
            if type(end_copy['oldValue']) == int:
                inner_index +=1
                continue
            oldVal, newVal = get_common_values_and_unique_values(end_copy['oldValue'], end_copy['newValue'])
            
            
            end['oldValue'] = oldVal
            end['newValue'] = newVal
            inner_index +=1
    if 'drivers.values' in endorsement_keys:
        while inner_index_2 < len(endorsements_json['drivers.values']['values']):
            end = endorsements_json['drivers.values']['values'][inner_index_2]
            end_copy = copy.copy(end)
            # if 'time' in list(end_copy.keys()):
            #     del end_copy['time']
