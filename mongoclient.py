...
for result in results:
    ...
    for index, driver in enumerate(drivers):
        ...
        if driver['driverEffDate'] != policy_json['policy']['effectiveDate'] and driver['driverEffDate'] != None:
            end_drivers = [driver]
            ...
            for dr in drivers:
                if dr not in end_drivers and driver['driverEffDate'] == dr['driverEffDate'] and driver['driverFirstName'] != dr['driverFirstName'] and driver['driverLastName'] != dr['driverLastName']:
             
                    end_drivers.append(dr)

            index_string = 'drivers.values'
            # endorsements_json[]
            indicator = False
            ...
            if index_string in endorsements_json:
                ...
                if indicator == False:
                    ...
                    print(driver['driverEffDate'], policyNum)
                    try:
                        older_drivers = [item for item in drivers if datetime.datetime.strptime(item['driverEffDate'].replace('//','/'), '%m/%d/%Y') < datetime.datetime.strptime(driver['driverEffDate'].replace('//','/'), '%m/%d/%Y')]
                    except:
                        print(policyNum)
                        continue
                    ...
                    endorsements_json[index_string]['values'].append({
                        'oldValue': older_drivers,
                        'newValue': older_drivers+end_drivers,
                        'time': datetime.datetime.strptime(driver['driverEffDate'].replace('//','/').strip(), '%m/%d/%Y').replace(hour=6, minute=0, second=0, microsecond=0).strftime('%Y-%m-%dT%H:%M:%S.%f'),
                        'effDate': driver['driverEffDate']
                    })

            else:
                
                print(driver['driverEffDate'])
                older_drivers = [item for item in drivers if datetime.datetime.strptime(item['driverEffDate'].replace('//','/'), '%m/%d/%Y') < datetime.datetime.strptime(driver['driverEffDate'].replace('//','/'), '%m/%d/%Y')]
                ...
                endorsements_json[index_string] = { 'values' : [{
                    'oldValue': older_drivers,
                    'newValue': older_drivers+end_drivers,
                    'time': datetime.datetime.strptime(driver['driverEffDate'].strip(), '%m/%d/%Y').replace(hour=6, minute=0, second=0, microsecond=0).strftime('%Y-%m-%dT%H:%M:%S.%f'),
                    'effDate': driver['driverEffDate']
                }]}
            ...
            #     }
    try:               
        for end in endorsements_json['drivers.values']['values']:
            if len(end['oldValue']) == len(end['newValue']):
                endorsements_json['drivers.values']['values'].remove(end)
    except:
        continue
