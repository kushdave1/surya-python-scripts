...
for result in results:
    ...
    # if policyNum != "23CAN00362":
    #     continue
    print(policyNum, vehicles)
    ...
    for index1, vehicle1 in enumerate(vehicles):
        ...
        if vehicle1['pedPipProtectionPremium'] == 'null':
            vehicle1['pedPipProtectionPremium'] = 0.0
        if vehicle1['uninsuredMotoristPremium'] == 'null':
            vehicle1['uninsuredMotoristPremium'] = 0.0
        if vehicle1['underinsuredMotoristPremium'] == 'null':
            vehicle1['underinsuredMotoristPremium'] = 0.0
        
        
        try:
            vehicle1['baseEffDate'] = format_date(vehicle1['baseEffDate'])
            vehicle1['baseExpDate'] = format_date(vehicle1['baseExpDate'])
            days_between = datetime.datetime.strptime(vehicle1['baseExpDate'], '%m/%d/%Y') - datetime.datetime.strptime(vehicle1['baseEffDate'], '%m/%d/%Y')
            print(days_between.days)
            if days_between.days < 365:
                vehicle1['overallPremium'] = round((float(coverage['overallPremium'])*(days_between.days/365)),2)
                vehicle1['pedPipProtectionPremium'] = round((float(coverage['pedPipProtectionPremium'])*(days_between.days/365)),2)
                vehicle1['uninsuredMotoristPremium'] = round((float(coverage['uninsuredMotoristPremium'])*(days_between.days/365)),2)
                vehicle1['underinsuredMotoristPremium'] = round((float(coverage['underinsuredMotoristPremium'])*(days_between.days/365)),2)
            
            else:
                vehicle1['pedPipProtectionPremium'] = coverage['pedPipProtectionPremium']
                vehicle1['uninsuredMotoristPremium'] = coverage['uninsuredMotoristPremium']
                vehicle1['underinsuredMotoristPremium'] = coverage['underinsuredMotoristPremium']
        except:
            continue
