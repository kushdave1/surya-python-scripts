...
for result in results:
    ...
    if 'drivers.values' in endorsement_keys:
        while inner_index < len(endorsements_json['drivers.values']['values']):
            end = endorsements_json['drivers.values']['values'][inner_index]
            ...
            for index2, end2 in enumerate(endorsements_json['drivers.values']['values']):
                ...
                if len(end_copy['oldValue']) == len(end2_copy['oldValue']) and len(end_copy['newValue']) == len(end2_copy['newValue']) and datetime.strptime(end_copy['effDate'], '%m/%d/%Y') == datetime.strptime(end2_copy['effDate'], '%m/%d/%Y') and inner_index != index2:
                    print('hi')
                    endorsements_json['drivers.values']['values'].remove(end2)
